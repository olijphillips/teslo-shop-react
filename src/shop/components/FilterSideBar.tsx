...
export const FilterSidebar = () => {
  ...
  const currentSizes = searchParams.get("sizes")?.split(",") || []; // xs, l, m, etc. Muy facil si da nulo entonces el resultado es undefined y se pasa []. Si NO es nulo es porque tiene algo.
  ...
  const handleSizeChanged = (size: string) => {
    ...
    searchParams.set("page", "1"); //actualizo la variable
    searchParams.set("sizes", newSizes.join(",")); //actualizo la variable
  ...
  const handlePriceChange = (price: string) => {
    searchParams.set("page", "1"); //actualizo la variable
    searchParams.set("price", price); //actualizo la variable
    setSearchParams(searchParams);
  };
